...
const convertCalendarItem = (googleCalendarItem) => {
  let calendarItem = {
    ...
    startTime: luxon(googleCalendarItem.start.dateTime).format('HH:mm'),
    endTime: luxon(googleCalendarItem.end.dateTime).format('HH:mm'),
    ...
    googleCalendar.events.list(
      {
        oauth2Clnt,
        calendarId: 'primary',
        timeMin: luxon().subtract(1, 'hours').toISOString(),
        timeMax: luxon().endOf('day').add(7, 'days').toISOString(),
        maxResults: 10,
        singleEvents: true,
        orderBy: 'startTime',
      },
